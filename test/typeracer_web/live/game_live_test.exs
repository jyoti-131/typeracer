defmodule TyperacerWeb.GameLiveTest do
  use TyperacerWeb.ConnCase
  ...
  describe "Index" do
    ...
    test "updates game in listing", %{conn: conn, game: game} do
      ...
      assert index_live |> element("#races-#{game.id} a", "Edit") |> render_click() =~
               "Edit Game"

      assert_patch(index_live, ~p"/races/#{game}/edit")
    ...
    test "deletes game in listing", %{conn: conn, game: game} do
      ...
      assert index_live |> element("#races-#{game.id} a", "Delete") |> render_click()
      refute has_element?(index_live, "#races-#{game.id}")
    end
  end
  ...
  describe "Show" do
    ...
    test "displays game", %{conn: conn, game: game} do
      {:ok, _show_live, html} = live(conn, ~p"/races/#{game}")

      assert html =~ "Show Game"
    end

    test "updates game within modal", %{conn: conn, game: game} do
      {:ok, show_live, _html} = live(conn, ~p"/races/#{game}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Game"

      assert_patch(show_live, ~p"/races/#{game}/show/edit")
      ...
      assert_patch(show_live, ~p"/races/#{game}")
